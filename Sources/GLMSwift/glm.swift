...
public struct GLMSwift {
    public static func translation<T: FloatingPoint>(vector v: Vector3<T>) -> Matrix4<T> {
    ...
    public static func rotation<T: BinaryFloatingPoint>(angle: T, axis v: Vector3<T>) -> Matrix4<T> {
        let a = angle
    ...
    public static func scale<T: FloatingPoint>(xScale: T, yScale: T, zScale: T) -> Matrix4<T> {
        Matrix4<T>(xScale, 0, 0, 0, 0, yScale, 0, 0,0, 0, zScale, 0,0, 0, 0, 1)
    }
